...
bool Spreadsheet::add(QList<QTableWidgetItem *> items, double& d)
{
    if(!items.isEmpty()) {
        foreach (auto item, items)
...
void Spreadsheet::ave()
{
    ...
    if(add(items, d)) {
        ...
        Cell* c = new Cell();
        c->setTextAlignment(Qt::AlignRight);
        c->setText((QString::number(d/selectedItems().size())));
        setItem(row, column, c);
        somethingChanged();
    }
}
...
void Spreadsheet::count()
{
    ...
    if(add(items, d)) {
        ...
        Cell* c = new Cell();
        c->setTextAlignment(Qt::AlignRight);
        c->setText((QString::number(selectedItems().size())));
        setItem(row, column, c);
        somethingChanged();
    }
}
...
void Spreadsheet::max()
{
    ...
    if(!items.isEmpty()) {
        ...
        double d = items[0]->text().toDouble();
        foreach (auto item, items)
            if(item->text().toDouble() > d)
                d = item->text().toDouble();
